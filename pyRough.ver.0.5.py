...
def DownwardUnionsOfClasses (infosystem):
    ...
    DecisionClass=[r[-1] for r in matrix]
    DecisionClass=list(set(DecisionClass))
    DownwardUnionClass=[[r for r in matrix if int(r[-1])<=c] for c in DecisionClass]
...
def UpwardUnionsOfClasses (infosystem):
    ...
    DecisionClass=[r[-1] for r in matrix]
    DecisionClass=list(set(DecisionClass))
    UpwardUnionClass=[[r for r in matrix if int(r[-1])>=c] for c in DecisionClass]
...
def ChoiceTypeElem(type_rule,c,e,preference,matrix):
    if type_rule=='one':
    ...
    while (len(G)!=0 ):
        ...
        while (len(rule)==0 or (set(obj_cov_by_rules)).issubset(B)==False):
            ...
            for c in range(1,criteria_num):
                ...
                for e in Cond:
                    elem=ChoiceTypeElem(type_rule,c,e,preference,matrix)
                    best=FindBestCondition(best, elem, rule, selected, G, infosystem)
...
def main():
    ...
    ##    infosystem=FileToInfoSystem("/home/gianluca/Documenti/modelli_GIS/RoughSet/Example_collaudo/Countries-Crit-5.isf")
        infosystem=FileToInfoSystem("Examples/Buses.isf")
